...
- an **interactive dashboard** (opens in your web browser) with your total value, how
  much leverage and risk you're carrying, your positions in a sortable table you can
  click to drill into, and charts; and
...
3. Connect the brokerage. **Pick ONE path**: SnapTrade (easiest, works for almost any
   broker including Robinhood and Interactive Brokers) or Interactive Brokers direct.
4. Install the Claude "skill" so `analyze my portfolio` just works.
5. **Personalize.** Offer to capture the user's own thesis and how they trade (including
...
`.env`. The secret is shown **once**. If it's lost, it must be reset, not recovered.
...
- Check **Read-Only API** (so nothing can trade)
...
**Claude: before the first analysis, offer to personalize, but don't force it. Say
...
- **Their thesis and what they're betting on:** the big-picture story (e.g. a specific
  theme, sector, or set of companies they believe in). Capture it in their words.
- **How concentrated or aggressive:** a few high-conviction names or broadly
...
- **How they use options.** This matters a lot for the read:
  ...
  - Do they **sell or write** options (covered calls, cash-secured puts, credit
    spreads, selling premium for income)? **If they sell premium, note it clearly.**
    Their short legs show up with negative or flipped delta, and their risk is about
...
**They can always skip now and just say _"let's personalize this"_ any time later**, and
...
Then read the newest `outputs/analysis_*.json`, view or describe the dashboard, and give
the user the candid position-by-position analysis per the skill.
...
- **SnapTrade: empty or weird positions**: re-run `scripts/analyze.py --debug` to dump
...
- **Prices show as `n/a` or P/L missing on a few options**: for Robinhood via SnapTrade,
...
- If you ever want to revoke access: for SnapTrade, disconnect the broker in the SnapTrade
  dashboard; for IBKR, turn off the API in Gateway settings.
